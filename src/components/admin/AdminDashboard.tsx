...
export function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    api.getDashboard()
      ...
      .catch(() => {
        // Dashboard charts remain illustrative; ignore stats fetch failure here.
      });
    return () => {
      active = false;
    };
  ...
  const stats = [
    ...
    {
      title: 'Voice Queries',
      value: (metrics?.voiceQueries ?? 0).toLocaleString(),
      change: 'Live',
      trend: 'up',
      icon: Mic,
      color: 'bg-green-500'
    ...
    {
      title: 'Text Queries',
      value: (metrics?.textQueries ?? 0).toLocaleString(),
      change: 'Live',
      trend: 'up',
      icon: FileText,
      color: 'bg-purple-500'
    },
    {
      ...
      value: (metrics?.flaggedQueries ?? 0).toLocaleString(),
      change: 'Live',
      trend: 'up',
      icon: Clock,
      color: 'bg-orange-500'
